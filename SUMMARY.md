...
---
## Deployment — Railway + Vercel · backend live

**Settled on Railway for the API and Vercel for the dashboard.** The API is deployed and
active at `triage-api-production-00b4.up.railway.app`. `DEPLOYMENT.md` is the single
deployment document; `scripts/verify_live.sh` is the check.

Render, Hugging Face Spaces and Azure App Service were each explored and abandoned.
Their artifacts — `render.yaml`, `AZURE.md`, `PRODUCTION-VIEW.md`, `hf-space/`,
`scripts/sync_hf_space.sh`, `railway.json` — are deleted rather than left to rot. Two
findings from that exploration outlived it and are the reason the current deploy works:

- **The port default was wrong.** `CMD … --port ${PORT:-8000}` shipped a container that
  listened on 8000 whenever the host didn't inject `$PORT`. Now `ENV PORT=7860` plus a
  shell-form `CMD … --port ${PORT}`, which is exactly right for Railway: it injects its
  own `PORT`, that overrides the image default, and the shell form expands it. *`PORT`
  must never be set as a Railway variable* — it would shadow the platform's value.
  *(The old `CMD ["sh","-c","…"]` array form did expand `${PORT}` correctly — exec-form
  arrays only fail to expand when they don't invoke a shell — so the wrong default was
  the whole of the defect.)*
- **CORS reads `ALLOWED_ORIGINS`**, comma-split at startup, `TRIAGE_CORS_ORIGINS` kept
  as a fallback. No hardcoded platform URL anywhere in `src/api/`.

Also from that work and still in place: `.dockerignore` (build context only — the
Dockerfile never did `COPY . .`; `eval/runs/*.db` explicitly re-included), and the
audit that every filesystem write in `src/` is either gated (`main.py`'s `init_db`
...
CLI-only. Proved empirically: 53 files snapshotted, every read route exercised plus a
refused write, tree byte-identical afterward.

**Verified against the live deployment, not localhost — 25/25, zero failures.**
`/health` 200 with `read_only: true`; coverage 27-of-110; `insufficient_funds` →
RETRY_SCHEDULED at 72h; all eight action classes through `POST /v1/recovery/decide`;
unknown code → 404 with the full envelope; `POST /v1/simulator/run` → **503**, so
`TRIAGE_READ_ONLY` is confirmed set on the platform; `/v1/rails/health`, `/v1/eval/runs`
and `/v1/eval/report/run_DbCttM8e` all 200.

**Needs a human.** Deploy the dashboard to Vercel (root directory `dashboard`,
`NEXT_PUBLIC_API_URL` = the Railway URL), then set `ALLOWED_ORIGINS` on Railway to that
Vercel domain plus `http://localhost:3000`. CORS is browser-enforced, so
`verify_live.sh` passes either way and cannot catch a missing origin — check the
browser console on the deployed Live screen instead.

...
| 5 | Dashboard + README + submission | **Done** — `dashboard/` (4 screens), README, architecture diagram, DEMO-SCRIPT, deployment. 544 tests + 131 verification assertions green |
...
### Stage 5 — Interface + submission · ~4h · **DONE**
- [x] Dashboard: Live (checkout + Inspector), Taxonomy, Results, Cases. Four screens.
- [x] `README.md` — finding, headline null with its diagnosis, results, losses, limitations
- [x] `docs/architecture.png` — regenerated by `scripts/make_diagram.py`
- [x] `DEMO-SCRIPT.md` — five minutes, the null at 2:30
- [x] `eval/PRODUCTION-CHECK.md` — A1–A5 + read-only, 175 checks, zero failures
- [x] Deployment: `Dockerfile`, `railway.toml`, read-only mode enforced two ways —
      API live on Railway, dashboard to Vercel. See `DEPLOYMENT.md`.
- [ ] Record the 5-minute video — the only thing left, and it is a recording, not a build
- **Fallback if the dashboard slips:** ship the CLI plus `eval/report-*.md`. The reports
  carry the evidence; the dashboard only presents it.
